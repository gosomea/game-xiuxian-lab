...
角色移动模块为 exploring，顶层入口 `src/levels/experiments/character_movement/movement_lab_hub.tscn`（顶层移动子实验目录，独立启动后 Esc 可返回顶层目录）：七项子实验中四项可运行——镜头实验室 `camera_lab.tscn`、人物动作工作台 `motion_stage.tscn`、移动庭院 `movement_garden.tscn`（小场景回归）、群山宗门 `mountain_realm.tscn`（180×160 m、五峰三落脚点，屏幕相对移动 + 独立跳跃 + 御剑飞行三能力与固定俯视跟随相机的综合场景）；其余三项（地形接触训练场、御剑飞行训练场、状态切换压力场）为 planned。子场景返回移动子实验目录。剑法及其余六个模块为 planned；移动场景不定义战斗规则。依据：[character-movement-garden](notes/implemented/gameplay/2026-09-18-character-movement-garden.md)、[character-movement-subexperiments](notes/proposed/gameplay/2026-09-18-character-movement-subexperiments.md)。启动入口仍为 `src/levels/lab_hub.tscn`，空白基底为 `src/levels/empty_stage.tscn`。先读 README 获取运行方式。
