...
> **本文件是「七场各自落地 / 集成」阶段的历史权威报告**（2026-09-18 上一阶段）。当前阶段的交付与验收
> 以 [可组合移动实验最终报告](2026-09-18-composable-movement-labs.md) 为准；本文件的计数与镜头/装配描述是
> 该阶段快照，**不再代表当前实现**（例如本文件的镜头策略、飞剑绑法与 HUD 结构均已被共享 CameraRig /
> FlightBundle / LabHud 取代）。七个子实验各自的报告同样保留其落地阶段的实测数字与结论，属**阶段快照**；
