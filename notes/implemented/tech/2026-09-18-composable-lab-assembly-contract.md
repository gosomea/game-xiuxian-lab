...
>
> **补充实现状态（2026-09-18 第二轮，已落地）**：本文件 §2/§3/§4 的「组合环绕模式」「未归属 RMB 消费策略」
> 「镜头输入组合测试契约」已实现并由测试覆盖：`CameraRig.handle_rmb_input()` 是 RMB 唯一早期入口
> （`_input` 中先于 GUI 与嵌入 Game 视图的世界区域 press 处理），`CameraRigConfig.consume_unowned_rmb`
> 默认 `false`、实验场景显式开启。§4 的五条契约由 `test_camera_rig_executor.gd`（默认放行 / fallback 只消费 /
> orbit 早捕获与释放恢复）+ `camera_lab_playtest.gd`（组合四输入同帧路径 / 早捕获 / 退出恢复 / 失焦恢复）+
> `character_movement_playtest.gd`（庭院场景级冒烟）覆盖，计数与命令见
> [镜头组合与 RMB 归属验收](../../../docs/playtest/2026-09-18-camera-combo-rmb.md)。
> **未完成**：编辑器嵌入 Game 视图的人工双形态验收仍待主代理执行（无头与独立窗口无法替代），
> 见同报告「嵌入窗口边界」。
