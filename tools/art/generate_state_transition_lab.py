...
cyl('Jade disc rim band', (0, 0, -0.06), DISC_R - 0.02, 0.12, bronze, verts=96)
...
GATE_Z = -5.5
for x in (-5.0, 5.0):
    box('Gate pillar', (x, GATE_Z, 1.9), (0.95, 1.05, 3.8), stone, .06)
box('Gate lintel', (0, GATE_Z, 4.05), (11.6, 1.25, 0.7), bronze, .05)
box('Gate wall slab', (0, GATE_Z, 1.5), (10.0, 0.55, 3.0), stone, .05)
box('Gate threshold', (0, GATE_Z + 0.9, 0.09), (10.6, 1.4, 0.18), paper, .04)
for x in (-3.3, -1.1, 1.1, 3.3):
    box('Gate rune strip', (x, GATE_Z + 0.60, 1.5), (0.14, 0.06, 2.2), rune_dim, 0.0)

# --- broken bridge due east (Godot +X) ---------------------------------------
BRIDGE_Z = 0.0
box('Bridge near span', (7.6, BRIDGE_Z, -0.22), (3.0, 2.6, 0.44), stone, .05)
box('Bridge far span', (13.1, BRIDGE_Z, -0.22), (3.2, 2.6, 0.44), stone, .05)
for x in (6.2, 9.05, 11.65, 14.65):
    box('Bridge rail post', (x, BRIDGE_Z, 0.55), (0.28, 0.28, 1.1), bronze, .04)
for x in (7.0, 9.0):
...
for index, (x, z, y, height) in enumerate(PILLARS):
    ...
    cyl('Pillar cap', (x, z, y + height / 2 + 0.12), 0.72, 0.24, bronze, verts=12)
